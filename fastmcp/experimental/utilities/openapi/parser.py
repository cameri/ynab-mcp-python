                ...
                for media_type_str, media_type_obj in request_body.content.items():
                    if (
                        ...
                        try:
                            ...
                            request_body_info.content_schema[
                                media_type_str
                            ] = schema_dict
                        except ValueError as e:
        ...
        if hasattr(self.openapi, "components") and self.openapi.components:
            ...
            if hasattr(components, "schemas") and components.schemas:
                for name, schema in components.schemas.items():
                    # logger.info(f"Processing schema definition '{name}'")
                    try:
                        if isinstance(schema, self.reference_cls):
                            ...
                            # logger.info(
                            #     f"Resolved schema reference '{name}' to: {schema_definitions[name]}"
                            # )
                        else:
                            schema_definitions[name] = self._extract_schema_as_dict(
                                schema
                            ...
                            # logger.info(
                            #     f"Extracted schema definition '{name}': {schema_definitions[name]}"
                            # )
                    except Exception as e:
                        logger.warning(
                            f"Failed to extract schema definition '{name}': {e}"
                        )
        # logger.info(f"Extracted schema_definitions: {json.dumps(schema_definitions)}")
